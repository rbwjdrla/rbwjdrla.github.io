...
팩토리 메서드(Factory Method) 패턴은 **객체 생성**을 전담하는 디자인 패턴 중 하나이며, **생성(Creational) 패턴**에 속함. 이 패턴의 핵심 아이디어는 **객체를 생성하는 인터페이스는 정의하되, 어떤 클래스의 인스턴스를 생성할지는 서브클래스에서 결정하도록 위임하는 것**임.
...
## 2. 문제점 (Problem it Solves)

일반적으로 객체를 생성할 때 `new` 키워드를 사용하여 특정 클래스의 인스턴스를 직접 생성함. 하지만 이 방식은 다음과 같은 문제를 야기할 수 있음:

* **강한 결합(Tight Coupling):** 클라이언트 코드가 구체적인 클래스에 직접적으로 의존하게 되어, 특정 클래스 구현이 변경되거나 새로운 클래스가 추가될 때마다 클라이언트 코드도 함께 수정해야 함.
